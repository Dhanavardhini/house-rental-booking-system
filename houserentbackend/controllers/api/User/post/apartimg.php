...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_FILES['file'])) {
        ...
        // Get other POST data
        $title = $_POST['title'] ?? ''; // Fix: Add $title here
        $description = $_POST['description'] ?? ''; // Fix: Add $description here
        $price = $_POST['price'] ?? ''; // Fix: Add $price here

        if (empty($title) || empty($description) || empty($price)) {
            handleResponse(400, 'Invalid input. Please provide title, description, and price.');
        }

        // Call the apartmentimg method
        $obj = new Post();
        $result = $obj->apartmentimg($title, $description, $price, $uploadPath);
